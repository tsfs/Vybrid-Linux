/*
 *  L2 switch Controller (Etheren switch) driver for MVF family.
 * 
...
int esw_set_vlan_id(struct switch_enet_private *fep,
...
static const struct net_device_ops mvf_switch_ops = {
	...
	int slot, struct platform_device *pdev)
{
	...
	volatile switch_t	*fecp;
	int 		i, j;
	struct mvf_switch_platform_data *plat =
...
//#endif
	...
	*/
#ifdef CONFIG_ARCH_MVF
	fecp = (volatile switch_t *)ioremap(plat->switch_hw[0], SZ_4K);
#else
	...
	//	new add
#ifdef CONFIG_ARCH_MVF
	fep->fec[0] = ioremap(plat->fec_hw[0], SZ_4K);
	fep->fec[1] = ioremap(plat->fec_hw[1], SZ_4K);
#endif
	...
	int slot)
{
	...
	if (slot == 0) {
		fep->mdio_bus->name = "MVF switch MII 0 Bus";
		strcpy(fep->mdio_bus->id, "0");
	} else if (slot == 1) {
		fep->mdio_bus->name = "MVF switch MII 1 Bus";
		strcpy(fep->mdio_bus->id, "1");
	} else {
		printk(KERN_ERR "Now mvf can not"
...
static int __init mvf_switch_init(void)
{
	return platform_driver_register(&eth_switch_driver);;
}

static void __exit mvf_switch_exit(void)
{
	platform_driver_unregister(&eth_switch_driver);
}

module_init(mvf_switch_init);
module_exit(mvf_switch_exit);
MODULE_LICENSE("GPL");
